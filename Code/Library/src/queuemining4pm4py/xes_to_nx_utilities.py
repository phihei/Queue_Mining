...
def transform_dfg_to_directed_nx_graph(dfg, activities=None) -> nx.DiGraph:
    ...
    :param activities: list of activities corresponding to node labels
    :return: NetworkX DiGraph corresponding to dfg
    """
    """
    Transform DFG to directed NetworkX graph, adapted from existing function in PM4PY in dfg_utils.py.
...
def transform_xes_log_to_nxDiGraph(log, variant='dfg', integer_labels=False) -> nx.DiGraph:
    ...
    :param log: Eventlog
    :type log: pm4py.objects.log.obj.EventLog
    :param variant: either 'dfg' or 'inductive' choosing the method for the base graph
    :type variant: str
    :param integer_labels: whether to use integers as node labels or keep original labels
    :type integer_labels: bool
    :return: Networkx DiGraph corresponding to event log
