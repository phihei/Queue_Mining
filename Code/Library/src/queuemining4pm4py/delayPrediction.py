...
class DelayPredictor:
    ...
    def __init__(self, eventLog: pm4py.objects.Eventlog, queueEntryEvent, queueAbandonEvent, serviceStartEvent, serviceEndEvent):
    ...
    def __annotate(self):
        ...
        for trace in self.eventLog:
            for i in range(len(trace) - 1):
                if trace[i]["concept:name"] == self.queueEntryEvent and trace[i+1]["concept:name"] == self.serviceStartEvent:
    ...
    def getQLMPPrediction(self, customer, numServiceProviders, serviceProviderMeanTimes: datetime.timedelta, customerAbandonRate):
        customerTrace = None
        for trace in self.eventLog:
            if trace.attributes["concept:name"] == str(customer):
                customerTrace = trace
    ...
    def getAllQLMPPrediction(self, numServiceProviders, serviceProviderMeanTimes: datetime.timedelta, customerAbandonRate):
        result = dict()
        my = 1 / serviceProviderMeanTimes.total_seconds()
        for index, customer in enumerate(self.q):
            if index == 0:
