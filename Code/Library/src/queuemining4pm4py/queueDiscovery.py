...
def add_scheduledTimestamps(log, columns: list= None) -> pd.DataFrame:
    ...
    if columns is None:
        columns = ['case:concept:name', 'concept:name', 'start_timestamp', 'time:timestamp', 'scheduled_timestamp',
                   'org:resource']
    resource = True #needs to be False to work with Hospital Log otherwise True
        ...
        for trace in log:
            for i in range(len(trace) - 1):
                if (trace[i]['concept:name'], trace[i + 1]['concept:name']) in df_activities:
                    ...
                    # here we need to check for concurrent preceding activities
...
def add_scheduledTimestamps_single(log, columns: list= None) -> pd.DataFrame:
    """
       This function takes an PM4Py event log and a optional list of column headers as input and transforms the event log
       into a dataframe while calculating the actual scheduled time for each event. In this case we assume a finish-start
       dependency between all directly following activities. Additionally, we take the late finish of the preceding activity
    ...
    print('Computing scheduled timestamps and creating Dataframe. This may take a while...')
    columns = ['case:concept:name', 'concept:name', 'time:timestamp', 'scheduled_timestamp']
    for trace in log:
        for i in range(len(trace) - 1):
            if (trace[i]['concept:name'], trace[i + 1]['concept:name']) in df_activities:
                # finish-start dependency, earliest start is latest finish of (all) preceding activities (if AND join)
...
def getPrecedingActivities(log, activities=None):
    """
    ...
    :param log: PM4Py event log
    :param activities: set of activities that schould be checked for preceding activities
...
def queue(df_log, lifecycle:bool=True, only_cases:bool=False, len_queues:bool=False, plt_bar:bool=False, print_n_cases:bool=False):
    """
    ...
    :param lifecycle: Boolean, has be set to false, if event log has no lifecycle transitions
    ...
    :param only_cases: Boolean, if True only Case IDs will be returned in the Dataframe, not the corresponding timestamps
    ...
    for row in range(len(df_log)):
        ...
        if lifecycle and (df_log.at[row, 'start_timestamp'] - curr_Time).total_seconds() > 0:
        ...
        elif not lifecycle and (df_log.at[row, 'time:timestamp'] - curr_Time).total_seconds() > 0:
            activity = df_log.at[row, 'concept:name']
            case_id = df_log.at[row, 'case:concept:name']
            case_start = df_log.at[row, 'time:timestamp']
            queues[activity].append((case_id, case_start))
    ...
    if plt_bar:
        stats_df.plot.bar(x='time Points', xticks=[], xlabel='Timestamps', y='number of Cases', title='Number of queued Cases over Time')
        plt.show()
    if only_cases:
